...
{
    if (!\Auth::check() && $skip) {
        return 'https://ramkabirbharat.org/public/images/user/user.png'; // Update the base URL
    }
    ...
    if (getFileExistsCheck($media)) {
        $baseUrl = 'https://ramkabirbharat.org/public';
        $fullUrl = $media->getFullUrl();
        
        // Ensure that the URL doesn't start with 'http://localhost'
        if (strpos($fullUrl, 'http://localhost') === 0) {
            $fullUrl = $baseUrl . substr($fullUrl, strlen('http://localhost'));
        }

        return $fullUrl;
    } else {
        return 'https://ramkabirbharat.org/public'; // Provide a default URL if media is not found
    }
}


function getMediaUrl($model, $collection)
    {
        $media = $model->getFirstMedia($collection);
