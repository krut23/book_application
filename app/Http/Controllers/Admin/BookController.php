    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->format)) {
          if ($request->format == 'pdf') {
            $pdfMaxSize = 30 * 1024 * 1024; // 30MB in kilobytes
            $validation_string = 'mimes:pdf|file|max:' . $pdfMaxSize;
            $validation_message = 'File type must be in pdf format and should not exceed 30MB.';
